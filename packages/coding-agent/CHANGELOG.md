...
- GUI-created worktrees now honor the shared clone-first and isolation-backend settings.
...
- Added nine RPC commands for remote (GUI) management surfaces: `mcp_add`/`mcp_test`/`mcp_reauth`/`mcp_reauth_cancel` (MCP server lifecycle — locked atomic config writes with `/mcp` validation parity, throwaway-`MCPManager` connection probes that never touch the shared connection pool, and OAuth reauthorization driven over the existing `extension_ui_request` `open_url`/`input` frames with a single-claim in-flight mutex refused via `code:"oauth_busy"` and AbortController-based cancel), `marketplace_action` (add/remove/update/install/uninstall/upgrade/list_available via `MarketplaceManager` with `classifySource` source-form validation), and `get_plugin_detail`/`set_plugin_features`/`set_plugin_setting`/`delete_plugin_setting` (plugin feature and settings management with manifest-schema validation); every mutation runs the established MCP/plugin reload finish so remote clients see fresh state. `get_mcp_servers` rows now also carry `scope`/`command`/`url`/`authState`.
- Added six RPC commands for remote (GUI/ACP) clients: `retry` (retry the last failed assistant turn, TUI `/retry` parity), `clear_context` (in-place context reset keeping the session, TUI `/clear` parity — refused with code `busy` while streaming or a foreground execution runs), `abort_subagent`/`revive_subagent` (per-agent lifecycle via the agent registry + lifecycle manager, TUI Agent Hub `x`/`r` parity; refuses main agent and read-only advisor transcripts with a `reason`), and `write_local_paste` (persist a large paste into the session's `local://` store with agent-side name allocation so concurrent clients never collide).
- Added an optional `direction` parameter (`"forward" | "backward"`, default `"forward"`) to the `cycle_model` RPC command, exposing the existing `AgentSession.cycleModel(direction)` to remote clients (TUI Shift+Ctrl+P backward cycling parity).
- Added seven RPC commands for remote (GUI/ACP) clients: `list_foreign_sessions` and `import_foreign_session` (list Claude/Codex sessions and import one as a fresh OMP session copy via the existing foreign-session importer — `--from-claude/--from-codex` parity, background-dispatched), `fork_from` (write an independent new session file containing only the root→entryId path via the new `SessionManager.copyBranchToNewSession`, without switching), `switch_leaf` (move the active leaf in place via `session.navigateTree`, TUI tree-selector parity, with the full cancelled/reopenAsk/editorText result contract), and `get_command_arg_completions` (dynamic slash-command argument candidates for remote composers — MCP server names and /move directories).
- Added `allowArgs` and `hasDynamicArgCompletion` fields to the `get_available_commands` wire payload so remote composers can decide argument-completion behavior without a round trip.
- Added the missing `fork` response variant to the `RpcResponse` union (the `fork` case was live but its success frame was force-cast).
...
- Added `transcribe_audio` and `synthesize_speech` RPC commands exposing the local STT (whisper) and TTS pipelines to RPC clients, plus a `get_theme_colors` RPC that resolves a named theme (built-in or custom file) to concrete colors — powering the GUI's voice input/output and unified themes.
