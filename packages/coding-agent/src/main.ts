...
// Note: upstream re-applies protocol-host default setting overrides here
// (HOST_DEFAULTED_SETTING_PATHS / applyRpcDefaultSettingOverrides). Our fork
// deliberately removed them (23abccf650): they masked persisted RPC setting
// writes from the GUI host. Keep that removal on every upstream sync.
...
export async function runRootCommand(
	...
	if (mode === "acp") {
		...
		if (mode !== "rpc" && mode !== "rpc-ui" && !isInteractive && !session.model) {
			if (modelRegistryError) {
