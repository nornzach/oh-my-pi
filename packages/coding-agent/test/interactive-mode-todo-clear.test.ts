...
describe("InteractiveMode todo HUD persistence", () => {
	...
	it("clears a settled todo list immediately in the session and panel", () => {
		setTodoClearDelay(0);
		...
		// The auto-clear timer is session-owned (RPC/GUI sessions get the
		// todo_auto_clear event), so a delay of 0 clears the session list itself.
	...
	it("clears closed todos after the configured delay", async () => {
		...
		session.setTodoPhases([{ name: "Implementation", tasks: [{ content: "done task", status: "completed" }] }]);
		mode.setTodos(session.getTodoPhases());
		expect(renderTodos(mode)).toContain("done task");
		...
		// The session-owned timer clears the settled list and emits
		// todo_auto_clear; the panel picks the cleared state up on reload.
		expect(session.getTodoPhases()).toEqual([]);
